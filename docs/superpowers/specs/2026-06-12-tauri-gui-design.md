...
- **Log format — identical to Python:** JSON object mapping `filename` → `{hash, converted_at, output_file}` with `converted_at` as local-time ISO 8601 and `output_file` as absolute path. Log entries are keyed by basename only (same limitation as Python: two different files with the same basename collide).
